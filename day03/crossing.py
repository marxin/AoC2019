...
def parse(path):
    return [(x[0], int(x[1:])) for x in path.split(',')]

def distance(point):
    return abs(point[0]) + abs(point[1])

def trace(path, trace, multi, position):
    path = list(reversed(path))
    while any(path):
        command = path.pop()
        for i in range(command[1]):
            turn = moves[command[0]]
            position[0] += turn[0]
            position[1] += turn[1]
            p = tuple(position)
            if p in trace:
...
def trace_intersections(path, intersection):
    path = list(reversed(path))
    position = [0, 0]
    distances = {}
    steps = 0
    while any(path):
        ...
        for i in range(command[1]):
            ...
            position[1] += turn[1]
            p = tuple(position)
            # print('[%d,%d]: %d' % (position[0], position[1], steps))
...
print(p1)
print(p2)
trace(p1.copy(), trace1, multi1, [0, 0])
print()
trace(p2.copy(), trace2, multi2, [0, 0])

intersection = set(trace1.intersection(trace2))

min = 2**32
for i in intersection:
    assert not i in multi1
    assert not i in multi2
    d = distance(i)
    if d < min:
        min = d
...
print(intersection)
distances1 = trace_intersections(p1.copy(), intersection)
print()
distances2 = trace_intersections(p2.copy(), intersection)
d1 = distances1
d2 = distances2
